...
from neural_bandit_model import NeuralBanditModelVAE
...
class NeuralLinearPosteriorSampling:
  ...
  def __init__(self, storage,  device, hparams):
    ...
    self.data_h = storage
    self.model = NeuralBanditModelVAE(hparams).to(self.device)

    self.target_model = NeuralBanditModelVAE(hparams).to(self.device)
  ...
  def action(self, decison_set, obs, fragment, ref_point=None):
    ...
    #     values = network_values
        ...
        try:
          ...
          # Sampling could fail if covariance is not positive definite
          d = self.param_dim
          w_dist = MultivariateNormal(torch.zeros(d, device=self.device, dtype=self.dtype), torch.eye(d, device=self.device, dtype=self.dtype))
          w = w_dist.sample()
        # decison_set_latent = decison_set_latent[R <= self.decison_set_radius]
