...
class NeuralLinearPosteriorSampling:
  ...
  def __init__(self, storage,  device, hparams):
    ...
    self.t = 0
    self.training_steps = [0 for _ in range(self.fragments)]

    self.data_h = storage
    self.model = [NeuralBanditModel(hparams).to(self.device) for _ in range(self.fragments)]

    self.target_model = []
    for k in range(self.fragments):
        target_model = NeuralBanditModel(hparams).to(self.device)
        target_model.load_state_dict(self.model[k].state_dict())
    ...
    if self.optimizer_name == 'Adam':
        self.optimizer = [torch.optim.Adam(self.model[k].parameters(), lr=self.lr, weight_decay=1e-8) for k in range(self.fragments)]
    elif self.optimizer_name == 'RMSprop':
        self.optimizer = [torch.optim.RMSprop(self.model[k].parameters(), lr=self.lr, weight_decay=1e-8)  for k in range(self.fragments)]
    else:
        raise ValueError('optimizer name is unkown')
    self.scheduler = [torch.optim.lr_scheduler.StepLR(self.optimizer[k], step_size=self.lr_step_size, gamma=self.lr_decay_rate) for k in range(self.fragments)]
  ...
  def add_bandit_fragment(self):
      ...
      model = NeuralBanditModel(self.hparams).to(self.device)
      target_model = NeuralBanditModel(self.hparams).to(self.device)
      ...
      if self.optimizer_name == 'Adam':
          ...
          optimizer = torch.optim.RMSprop(model.parameters(), lr=self.lr, weight_decay=1e-8)
      else:
          raise ValueError('optimizer name is unkown')
      scheduler = torch.optim.lr_scheduler.StepLR(optimizer, step_size=self.lr_step_size, gamma=self.lr_decay_rate)

      self.model.append(model)
      self.target_model.append(target_model)
      self.optimizer.append(optimizer)
      self.scheduler.append(scheduler)
      self.training_steps.append(0)
  ...
  def action(self, decison_set, obs, fragment, ref_point=None):
    ...
    with torch.no_grad():
      ...
      est_vals, decison_set_latent = self.model[fragment](obs, decison_set)
      self.est_vals = est_vals
      decison_set_latent = decison_set_latent.to(self.dtype)
      if ref_point is not None:
        ref_latent = self.model[fragment].encode(obs[:1], ref_point.unsqueeze(0))
  ...
  def update(self, exp, context, train=True):
    ...
    if self.t % self.update_freq_nn == 0 and self.t >= self.batch_size and train:
      ...
      for i in range(self.fragments):
          mean_loss = self.train(data, i)
          self.model[i].eval()
          self.precision[i] = self.lambda_prior * torch.eye(self.param_dim, device=self.device, dtype=self.dtype)
          self.f[i] = self.precision[i] @ self.model[i].get_last_layer_weights().to(self.dtype)#torch.zeros(self.param_dim, device=self.device, dtype=self.dtype)
      ...
      for j in range(len(data)):
          ...
          for i in range(self.fragments):
              ...
              if not any(step_idx == obs_idx):
                  continue
              with torch.no_grad():
                  states = torch.tensor(np.array(exp.obs[obs_idx:])).float()
                  context1 =  torch.stack([context for _ in range(len(states))])
                  target_values, _ = self.target_model[i](states.to(self.device), context1.to(self.device))
                  target_values = target_values[:, 0].unsqueeze(0)
                  returns = self.compute_returns_and_advantage(torch.tensor(exp.mask)[obs_idx:].unsqueeze(0),
                                                               torch.tensor(exp.reward)[obs_idx:].unsqueeze(0),
              ...
              contexts = context1[0:1].to(self.device)
              first_states = states[0:1].float().to(self.device)
              values  =returns[0:1].to(self.device)


              with torch.no_grad():
                  new_z = self.model[i].encode(first_states, contexts)
      ...
      for i in range(self.fragments):
        ...
        for i in range(self.fragments):
            ...
            if not any(step_idx == obs_idx):
                continue
            with torch.no_grad():
                states = torch.tensor(np.array(exp.obs[obs_idx:])).to(self.device).float()
                contexts = torch.stack([context for _ in range(len(states))])
                contexts = contexts.to(self.device)
                target_values, _ = self.target_model[i](states, contexts)
                target_values = target_values[:,0].unsqueeze(0)
                returns = self.compute_returns_and_advantage(torch.tensor(exp.mask[obs_idx:]).unsqueeze(0).to(self.device), torch.tensor(exp.reward)[obs_idx:].unsqueeze(0).to(self.device), target_values)
                ...
                #sample only first states:
                states1 = states[0:1]
                contexts1 = contexts[0:1]
                returns1 = returns[0:1]
                phi = self.model[i].encode(states1, contexts1)
                phi = phi.to(self.dtype)
                # Retrain the network on the original data (data_h)
                self.precision[i] += torch.matmul(phi.T, phi)
                self.f[i] += torch.matmul(returns1, phi)
                # self.cov[i] = torch.linalg.inv(self.precision[i])
                self.mu[i] = torch.linalg.solve(self.precision[i], self.f[i])
  ...
  def train(self, data, k):
        ...
        for i, sample in enumerate(dataloader):
            self.training_steps[k] += 1
            if self.training_steps[k] % self.target_model_update == 0:
                self.soft_update_from_to(self.model[k], self.target_model[k])
                self.target_model[k].eval()
            loss = self.train_step(sample, k)
            if loss is not None:
                num_iter += 1
                mean_loss += loss
  ...
  def train_step(self, sample, fragment):
      context, obs, _, reward, mask, steps = sample
      # context = torch.tile(context,[1,obs.shape[1],1])
      B = context.shape[0]
      obs_idx = fragment * self.horizon
      loss = 0
      cnt = 0
      obs_arr = []
      return_arr = []
      context_arr = []
      for b in range(B):
          if not any(steps[b] == obs_idx):
              continue
          obs1 = obs[b][obs_idx:].to(self.device)
          H = obs1.shape[0]
          if H == 1:
              return_arr.append(reward[b][obs_idx:].to(self.device))
          else:

              cnt += 1
              context1 = torch.stack([context[b] for _ in range(H)])  #  H x C

              reward1 = reward[b][obs_idx:].to(self.device)
              mask1 = mask[b][obs_idx:].to(self.device)
              context1 = context1.to(self.device)


              with torch.no_grad():
                  target_value, _ = self.target_model[fragment](obs1, context1)
          ...
          context_arr.append(context[b])

      if cnt < 2:
          return None
      context = torch.stack(context_arr).to(self.device)
      obs1 = torch.stack(obs_arr)
      returns = torch.cat(return_arr)
      value_hat, phi = self.model[fragment](obs1, context)
      loss += self.loss_fn(value_hat[:, 0], returns)  # + self.loss_fn(context,z_hat)
      loss += 0.1 * torch.mean(torch.norm(phi,dim=-1))

      loss = loss / cnt
      total_loss = loss.item()
      self.optimizer[fragment].zero_grad()
      loss.backward()
      torch.nn.utils.clip_grad_norm_(self.model[fragment].parameters(), self.max_grad_norm)
      self.optimizer[fragment].step()
      self.scheduler[fragment].step()
...
class NeuralLinearDataset(Dataset):
  ...
  def __getitem__(self, item):
    ...
    t = 0#np.random.randint(traj_len)
