...
class ARSLearner(object):
    ...
    def __init__(self, env_name='HalfCheetah-v1',
        ...
        self.latent_delta_std = params['latent_delta_std']
        ...
        self.w_size = len(self.w_policy.reshape(-1))
    ...
    def aggregate_rollouts(self, num_rollouts = None, evaluate = False):
        ...
        for _ in range(args.average_first_state):
                    curr_step_size = 0
                    idxes, deltas = self.deltas.get_deltas(self.bandit_algo.latent_dim, self.num_bandit_deltas)
                    deltas = (self.latent_delta_std  * deltas).reshape([len(idxes)] + list([self.bandit_algo.latent_dim]))
                    ...
                    # first_state = torch.stack([first_state for _ in range(self.num_bandit_deltas)])
                    first_state = first_state.float()
                    if self.params['filter'] == 'MeanStdFilter':
                        w = ray.get(self.workers[0].get_weights_plus_stats.remote())
                        ref_point = np.concatenate([self.w_policy.reshape(-1), w[1], w[2]])
                    else:
                        ref_point = self.w_policy.reshape(-1)
                    ref_point = torch.tensor(ref_point, device=self.device, dtype=torch.float)

                    p1, best_idx, values_plus, _ = self.bandit_algo.reverse_action(ref_point.unsqueeze(0), first_state.unsqueeze(0), deltas, fragment=frag_idx)
                    self.w_policy = p1[:,:self.w_size].reshape(self.w_policy.shape)
        t2 = time.time()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--latent_step_size',  type=float, default=10.0)
    parser.add_argument('--latent_delta_std',  type=float, default=0.01)
    ...
    parser.add_argument("--ucb_coeff", type=float, default=1.0)
    ...
    if args.n_directions > 50:
        ...
        args.training_iter = args.n_directions * 6
    else:
        args.training_freq_network = args.n_directions * 2 * 5
        args.training_iter = args.n_directions * 6 * 5
