    ...
    def do_rollouts(self, w_policy, num_rollouts = 1, shift = 1, evaluate = False, idxes=None, eval_new=False):
        ...
        for i in range(num_rollouts):
            ...
            if evaluate:
                ...
                self.policy.update_weights(w_policy)
                # self.policy.update_weights(w_policy)
                reward, steps1, obss, actions, rewards, masks, step_id = self.rollout(
                    shift=shift)
                steps += steps1
                rollout_rewards.append(reward)
                obss_arr.append(obss)
...
class ARSLearner(object):
    ...
    def __init__(self, env_name='HalfCheetah-v1',
        ...
        # for i in range(len(self.meta_policy_set)):
        #     for j in range(self.num_workers):
        #         self.meta_policy_set[i].observation_filter.update(ray.get(self.workers[j].get_filter.remote()))
        #     self.meta_policy_set[i].observation_filter.stats_increment()
        #     self.meta_policy_set[i].observation_filter.clear_buffer()
        # # sync all workers
        ...
        self.w_policy = self.meta_set[id].copy()
    ...
    def aggregate_rollouts(self, num_rollouts = None, evaluate = False):
        ...
        ## Bandits from here
        # with torch.no_grad():
        #     # for p in self.policy_history_set:
        ...
        #
        #                 decison_set_pos = self.w_policy + deltas
        #                 decison_set_neg = self.w_policy - deltas
        ...
        self.w_policy -= self.optimizer._compute_step(g_hat).reshape(self.w_policy.shape)
        self.meta_set.append(self.w_policy)
        self.meta_set.pop(0)
        # policy_id = ray.put(self.w_policy)
        # rollout_ids = [worker.do_rollouts.remote(policy_id,
        #                                          num_rollouts=1,
        ...
        with torch.no_grad():
                ...
                self.curr_idx = best_idx_plus
                self.w_policy = self.meta_set[self.curr_idx].copy()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--meta_set_size", type=int, default=1024)
