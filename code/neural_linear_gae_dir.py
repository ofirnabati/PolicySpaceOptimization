...
from neural_bandit_model import NeuralBanditModelDirichlet
...
class NeuralLinearPosteriorSampling:
  ...
  def __init__(self, storage,  device, hparams):
    ...
    self.data_h = storage
    self.model = NeuralBanditModelDirichlet(hparams).to(self.device)

    self.target_model = NeuralBanditModelDirichlet(hparams).to(self.device)
  ...
  def find_parameters(self, initial_point, latent, obs, fixed_parameters=None, cal_R=False):
      ...
      for _ in range(1000):
        ...
        if fixed_parameters is None:
            ...
            phi_hat = self.model.sample(obs, torch.cat([x,y],dim=-1))
        loss = self.mse_loss(phi_hat,latent)
        self.model.zero_grad()
        loss.backward()
        grad = x.grad.data.detach()
        x = x.detach() - 0.00784 * grad
        if cal_R:
            R = x - initial_point
  ...
  def reverse_action(self, policy, obs, deltas, fragment):
      ...
      with torch.no_grad():
          ...
          if self.method == 'ucb':
              d = self.latent_dim
              self.ucb = torch.sqrt(torch.sum(torch.linalg.solve(self.precision[fragment],
                                                                 decison_set_latent_pos.T).T * decison_set_latent_pos, dim=1))
              ...
              ucb_neg = torch.sqrt(torch.sum(torch.linalg.solve(self.precision[fragment],
                                              decison_set_latent_neg.T).T * decison_set_latent_neg, dim=1))
              ...
              try:
                  ...
                  d = self.param_dim
                  w_dist = MultivariateNormal(self.mu[fragment], torch.eye(d, device=self.device))
                  w = w_dist.sample()
  ...
  def compute_loss(self, policy,obs,fragment):
      self.model.eval()
      with torch.no_grad():
          # network_values, decison_set_latent = self.model(obs, decison_set)
          # decison_set_latent = self.model.encode(obs, decison_set)
          ...
          if self.method == 'ucb':
              d = self.latent_dim
              ucb = torch.sqrt(torch.sum(torch.linalg.solve(self.precision[fragment].float(),
                                                                 x.T).T * x, dim=1))
              ls_values = x @ self.mu[fragment].float()
              self.ls_values = ls_values.detach()
              self.ucb = ucb.detach()
              values = ls_values + self.ucb_coeff * ucb
          elif self.method == 'ts':
              try:
                  w_dist = MultivariateNormal(self.mu[fragment],
                                              precision_matrix=(1 / self.sigma2_s) * self.precision[fragment])
                  ...
                  w = w_dist.sample()
              # decison_set_latent = decison_set_latent[R <= self.decison_set_radius]
              # values = torch.matmul(latent, w.float())
              latent += self.step_size * (w / w.norm())
          else:
              raise ValueError('method is unknown')
          # self.model.zero_grad()
          # values.backward()
          # torch.nn.utils.clip_grad_norm_(x, self.max_grad_norm)
          # grad = x.grad.data.detach()
          # x = x.detach() + self.step_size * grad

      best_parameters = self.find_parameters(policy, latent, obs)
      best_parameters = best_parameters.cpu().numpy()
      ...
      for j in range(len(data)):
          ...
          # for i in range(self.fragments):
              ...
              for h in range(len(states) // self.horizon + 1):
                  idx = np.random.randint(len(states))# if self.state_based_value else 0
                  # idx = h * self.horizon
                  returns = np.sum(
                  ...
                  returns = torch.tensor(returns, device=self.device)
                  values = returns.unsqueeze(0).double()  # torch.tensor(returns, device=self.device)
      ...
      self.b[i] = self.b0 + b_upd
        ...
        with torch.no_grad():
            ...
            for h in range(len(states) // self.horizon  + 1):
                ...
                returns = torch.tensor(returns, device=self.device)
                returns = returns.unsqueeze(0).double() #torch.tensor(returns,device=self.device)
  ...
  #             return_arr.append(returns[0])
  #             obs_arr.append(obs1[:-1])
  #             context_arr.append(context1[:-1])
  #     context = torch.cat(context_arr).to(self.device)
  #     obs1 = torch.cat(obs_arr)
  ...
  def train_step(self, sample):
      ...
      prior_mean = self.model.prior_mean.expand_as(mu)
      prior_var = self.model.prior_var.expand_as(log_var)
      prior_logvar = self.model.prior_logvar.expand_as(log_var)
      var_division = log_var.exp() / prior_var  # Σ_0 / Σ_1
      diff = mu - prior_mean  # μ_１ - μ_0
      diff_term = diff * diff / prior_var  # (μ_1 - μ_0)(μ_1 - μ_0)/Σ_1
      logvar_division = prior_logvar - log_var  # log|Σ_1| - log|Σ_0| = log(|Σ_1|/|Σ_2|)
      kld_loss = 0.5 * ((var_division + diff_term + logvar_division).sum(dim=1) - mu.shape[-1])
      ...
      loss = likelihood + self.kld_coeff * kld_loss.mean()
...
class NeuralLinearDataset(Dataset):
  ...
  def __getitem__(self, item):
    ...
    t = np.random.randint(traj_len)# if self.state_based else 0
