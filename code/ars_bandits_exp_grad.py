...
class ARSLearner(object):
    ...
    def aggregate_rollouts(self, num_rollouts = None, evaluate = False):
        ...
        deltas = ((self.delta_std )  * deltas).reshape([len(idxes)] + list([self.bandit_algo.latent_dim]))
        ...
        first_state = first_state.unsqueeze(0)
        ref_point = self.bandit_algo.compute_loss(ref_point, first_state, frag_idx)

        if self.params['filter'] == 'MeanStdFilter':
            self.w_policy = ref_point[:, :-(2 * self.ob_size)].reshape(self.w_policy.shape)
        else:
            self.w_policy = ref_point.reshape(self.w_policy.shape)
