...
class NeuralLinearPosteriorSampling:
  ...
  def __init__(self, storage,  device, hparams):
    ...
    self.eff_horizon = (1 / (1 - self.gamma)) if self.gamma < 1.0 else 1000.0
  ...
  def find_parameters(self, initial_point, latent, obs, fixed_parameters=None, cal_R=False):
      ...
      for _ in range(1000):
        ...
        if fixed_parameters is None:
            phi_hat = self.model.encode(obs,x)
        else:
      ...
      for j in range(len(data)):
          ...
          # for i in range(self.fragments):
              ...
              for h in range(len(states) // self.horizon + 1):
                  ...
                  returns = (torch.tensor(returns, device=self.device) + self.eff_horizon) / (2 * self.eff_horizon)
  ...
  def train_step(self, sample):
      ...
      returns = (torch.cat(return_arr) * self.eff_horizon) / (2 * self.eff_horizon)
      value_hat, phi = self.model(obs1, context)
      loss = self.loss_fn(value_hat[:, 0], returns)  # + self.loss_fn(context,z_hat)
      # loss += 0.1 * torch.mean(torch.norm(phi,dim=-1))
