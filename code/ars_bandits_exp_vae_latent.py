    ...
    def aggregate_rollouts(self, num_rollouts = None, evaluate = False):
        ...
        if self.t < 3:
            ...
            for _ in range(args.average_first_state):
                        ...
                        deltas = (self.delta_bandit_std  * deltas).reshape([len(idxes)] + list([self.bandit_algo.latent_dim]))
                        ...
                        if self.params['filter'] == 'MeanStdFilter':
                            self.w_policy = p1[:,:-(2 * self.ob_size)].reshape(self.w_policy.shape)
                        else:
                            self.w_policy = p1.reshape(self.w_policy.shape)
...
def run_ars(args):
    ...
    wandb.run.name = args.env_name + '_' + 'neural_es_vae_latent'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--latent_step_size',  type=float, default=0.05)
    parser.add_argument('--delta_std', '-std', type=float, default=.03)
    parser.add_argument('--delta_bandit_std',  type=float, default=1.0)
    parser.add_argument('--n_workers', '-e', type=int, default=18)
    parser.add_argument('--rollout_length', '-r', type=int, default=1000)
    parser.add_argument('--average_first_state', type=int, default=3)
    ...
    args.training_iter = args.n_directions * 2 * 3 * 5
